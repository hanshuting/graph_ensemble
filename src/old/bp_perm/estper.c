...
void printMatrix(double **W, int n)
  ...
  for (i=0; i<n; i++) {
	...
	THRESHOLD) {
      printf("%f/%f\n",
	     ...
	     node->neighbors[i]->prevmessage[node->id][node->neighbors[i]->id]);printf("numerator below threshold? %d\n", node->f[node->neighbors[i]->id]<THRESHOLD);
	     }*/
    ...
    if (isnan(sum)) {
      printf("Warning. sum of message is %f\n", sum);
...
int permCheck(node_t **nodes, int n,double convergenceThreshold)
...
void writeBeliefs(double** beliefs,int n,char * filename){
  FILE *fout;
  char filename2[100];
  sprintf(filename2,"%s",filename);
   
  //  printf("writing to %s\n",filename2);
  fout = fopen(filename2, "wt");
  //  fout = fopen(filename, "wt");
  int i,j;

  for (i=0; i<n; i++) {
    for(j = 0; j < n; j++){
      fprintf(fout,"%f ",beliefs[i][j]);
    }
    fprintf(fout,"\n");
...
double computeBethe(node_t **X, node_t **Y, int n, int returnBeliefs, double** beliefs) {
...
int main(int argc, char**argv)
{
  ...
  node_t **alpha, **beta;
  if (argc==5 || argc == 6) {
    int printBeliefs = argc == 6;
    double convergenceThreshold = atof(argv[4]);
    n = atoi(argv[2]);
    double** beliefs;
    ...
    if(printBeliefs){
      beliefs = (double**)malloc(n*sizeof(double*));
      for (i=0; i<n; i++) {
	beliefs[i] = (double*)malloc(n*sizeof(double));
      }
    }
    ...
    *********************************************/
	...
	updateMessages(beta[i],n);
      ...
      converged -= permCheck(alpha,n,convergenceThreshold);
      //printf("Checked for convergence == %d\n", converged);
	...
	//printf("Bethe at iteration %d is %f\n", iters, computeBethe(alpha, beta,n));
	damp = damp*.99;
      }
      
      //printf("Message x_1->y_2 %f %f %f %f %f\n", 
    ...
    *********************************************/
    ...
    double b1 = computeBethe(alpha,beta,n,printBeliefs,beliefs);
    double bethe = exp(-b1);
    if(printBeliefs){
      writeBeliefs(beliefs,n,argv[5]);
    }
    writeOut(bethe, iters,argv[3]);
    ...
    printf("Usage: ./estper <input file> <size> <output file>\n");
  }
