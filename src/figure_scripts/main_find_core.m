...
%% load data
fpathbase = 'C:\Shuting\graph_ensemble\';
expt_name = 'G4M4_exp6';
rawdata_file = [fpathbase 'data\' expt_name '\' expt_name '.mat'];
data_file = [fpathbase 'data\' expt_name '\' expt_name '_high_add_neuron.mat'];
model_file = [fpathbase 'results\' expt_name '\' expt_name '_high_add_neuron_loopy_best_model_full.mat'];
shuffle_file = [fpathbase 'results\' expt_name '\shuffled_' expt_name '_high_add_neuron_loopy_fulldata.mat'];
spath = [fpathbase 'results\' expt_name '\'];

load(data_file);
load(rawdata_file);
best_model = load(model_file);
shuffle_model = load(shuffle_file);

%% pre-process
vis_stim = vis_stim(Pks_Frame);
data = data';
...
%% plot graph model
coords = Coord_active;
coords(end+1,:) = [min(coords(:,1)),min(coords(:,2))];
coords(end+1,:) = [min(coords(:,1)),max(coords(:,2))];
coords(end+1,:) = [max(coords(:,1)),min(coords(:,2))];

figure; set(gcf,'color','w')
% plotGraphModel2(best_model.graph,coords,best_model.edge_pot,[],jet(64));
plotGraphModelHighlightEP(best_model.graph,coords,best_model.ep_on,...
    [-0.1 0.03],[],gray(64),[]);

saveas(gcf,[spath expt_name '_model.fig']);
...
saveas(gcf,[spath expt_name '_core.fig']);
save([spath expt_name '_core.mat'],'core_crf');
