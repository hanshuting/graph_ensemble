function [] = figS2_SVD_OSI_louvain(param)
...
num_crf_osi_shared = zeros(num_expt,1);
num_crf_svd_shared = zeros(num_expt,1);
num_crf_louvain_shared = zeros(num_expt,1);

num_crf = zeros(num_expt,1);
num_osi = zeros(num_expt,1);
num_svd = zeros(num_expt,1);
num_louvain = zeros(num_expt,1);
...
%     end
    ...
    load([result_path_base '\' expt_name{n} '\core\' expt_ee '_louvain_core.mat']);
    ...
    for ss = 1:num_stim
        ...
        crf_louvain = intersect(core_crf{ss},louvain_core{ss});
        num_cell(expt_count) = size(Spikes,1);
        num_crf(expt_count) = length(core_crf{ss});
        num_osi(expt_count) = length(core_osi{ss});
        num_louvain(expt_count) = length(louvain_core{ss});
        num_crf_osi_shared(expt_count) = length(crf_osi);
        num_crf_louvain_shared(expt_count) = length(crf_louvain);
        num_crf_osi_total(expt_count) = length(unique(setdiff([core_crf{ss};core_osi{ss}],...
            crf_osi)));
        num_crf_louvain_total(expt_count) = length(unique(setdiff([core_crf{ss};louvain_core{ss}],...
            crf_louvain)));
        ...
        % louvain
        [pred.louvain{expt_count},cos_sim.louvain{expt_count},cos_thresh.louvain(expt_count),...
            cos_sim_avg.louvain(expt_count,:),acc,prc,rec] = core_cos_sim(louvain_core{ss},...
            data_high,true_label{expt_count});
        pred_stats.louvain(expt_count,:) = [acc,prc,rec];
        
        subplot(3,num_stim,ss);
        plotCoreOverlay(Coord_active,core_crf{ss},core_svd{ss},mycc.orange,...
            mycc.green,rr)
        subplot(3,num_stim,num_stim+ss);
        plotCoreOverlay(Coord_active,core_crf{ss},louvain_core{ss},mycc.orange,...
            mycc.purple,rr)
        subplot(3,num_stim,num_stim*2+ss);
        ...
        if ~isempty(core_svd{ss})
            ...
            num_crf_svd_shared(svd_expt_count) = length(crf_svd);
            num_svd(svd_expt_count) = length(core_svd{ss});
            num_crf_svd_total(expt_count) = length(unique(setdiff([core_crf{ss};core_svd{ss}],...
                crf_svd)));
...
setBoxStyle(h,linew);
h = boxplot(num_louvain./num_cell,'positions',3*stepsz,'width',ww,'colors',mycc.purple);
setBoxStyle(h,linew);
h = boxplot(num_osi./num_cell,'positions',4*stepsz,'width',ww,'colors',mycc.gray);
setBoxStyle(h,linew);
xlim([0 5*stepsz])
...
pval(2) = ranksum(num_crf./num_cell,num_louvain./num_cell);
pval(3) = ranksum(num_crf./num_cell,num_osi./num_cell);
title(num2str(pval))

% plot shared number
subplot(1,2,2); hold on
h = boxplot(num_crf_svd_shared./num_crf(svd_indx==1),'positions',stepsz,'width',ww,'colors',mycc.green);
setBoxStyle(h,linew);
h = boxplot(num_crf_louvain_shared./num_louvain,'positions',2*stepsz,'width',ww,'colors',mycc.purple);
setBoxStyle(h,linew);
h = boxplot(num_crf_osi_shared./num_crf,'positions',3*stepsz,'width',ww,'colors',mycc.gray);
setBoxStyle(h,linew);
xlim([0 4*stepsz])
ylim([0 1])
...
saveas(gcf,[fig_path expt_ee '_' savestr '_svd_louvain_osi_core_nums.pdf'])
...
for ii = 1:expt_count
    ...
    % louvain
    [auc.louvain(ii),xx.louvain{ii},yy.louvain{ii}] = plotROC(true_label{ii},...
    cos_sim.louvain{ii}',1,mycc.purple_light,linew);
    [~,uid] = unique(xx.louvain{ii});
    ymat.louvain(ii,:) = interp1(xx.louvain{ii}(uid),yy.louvain{ii}(uid),xvec);
...
plot(xvec,mean(ymat.louvain,1),'color',mycc.purple,'linewidth',2*linew)
plot(xvec,mean(ymat.osi,1),'color',mycc.gray,'linewidth',2*linew)
xlim([0 1]); ylim([0 1]);
set(gca,'linewidth',linew)
legend('off')

saveas(gcf,[fig_path expt_ee '_' savestr '_svd_osi_louvain_roc_curve.pdf'])
...
% legend('off')
% 
% saveas(gcf,[fig_path expt_ee '_' savestr '_louvain_roc_curve.pdf'])
...
% for ii = 1:expt_count
%     [auc.crf(ii),xx.crf{ii},yy.crf{ii}] = plotROC(true_label{ii},...
%         cos_sim.osi{ii}',1,mycc.orange_light,linew);
%     [~,uid] = unique(xx.crf{ii});
%     ymat.crf(ii,:) = interp1(xx.crf{ii}(uid),yy.crf{ii}(uid),xvec);
% %     if svd_indx(ii)==1
% %         svd_count = svd_count+1;
...
% scatter((3*stepsz-binsz)*ones(size(cos_sim_avg.louvain(:,1))),cos_sim_avg.louvain(:,1),...
%     30,mycc.purple_light,'+','linewidth',linew);
% scatter((3*stepsz+binsz)*ones(size(cos_sim_avg.louvain(:,2))),cos_sim_avg.louvain(:,2),...
%     30,mycc.purple,'+','linewidth',linew);
% scatter((4*stepsz-binsz)*ones(size(cos_sim_avg.osi(:,1))),cos_sim_avg.osi(:,1),...
%     30,mycc.gray_light,'+','linewidth',linew);
% scatter((4*stepsz+binsz)*ones(size(cos_sim_avg.osi(:,2))),cos_sim_avg.osi(:,2),...
...
%     mycc.black,'linewidth',3*linew);
% % louvain mean
% plot([(3*stepsz-binsz)*ones(size(cos_sim_avg.louvain(:,1))),(3*stepsz+binsz)*...
%     ones(size(cos_sim_avg.louvain(:,1)))]',cos_sim_avg.louvain','color',mycc.purple);
% plot(3*stepsz-binsz*[1.5 0.5],nanmean(cos_sim_avg.louvain(:,1))*ones(2,1),'color',...
%     mycc.black,'linewidth',3*linew);
% plot(3*stepsz+binsz*[1.5 0.5],nanmean(cos_sim_avg.louvain(:,2))*ones(2,1),'color',...
%     mycc.black,'linewidth',3*linew);
% % osi mean
% plot([(4*stepsz-binsz)*ones(size(cos_sim_avg.osi(:,1))),(4*stepsz+binsz)*...
%     ones(size(cos_sim_avg.osi(:,1)))]',cos_sim_avg.osi','color',mycc.gray);
% plot(4*stepsz-binsz*[1.5 0.5],nanmean(cos_sim_avg.osi(:,1))*ones(2,1),'color',...
%     mycc.black,'linewidth',3*linew);
% plot(4*stepsz+binsz*[1.5 0.5],nanmean(cos_sim_avg.osi(:,2))*ones(2,1),'color',...
%     mycc.black,'linewidth',3*linew);
% xlim([0.2 5*stepsz-0.2])
% set(gca,'xcolor','w')
% ylabel('Similarity')

% mean sim value, plot with box
subplot(1,5,1); hold on
h = boxplot(cos_sim_avg.crf(:,1),'positions',stepsz-binsz,'width',ww/2,'colors',mycc.orange_light);
setBoxStyle(h,linew)
h = boxplot(cos_sim_avg.crf(:,2),'positions',stepsz+binsz,'width',ww/2,'colors',mycc.orange);
setBoxStyle(h,linew)
h = boxplot(cos_sim_avg.svd(:,1),'positions',2*stepsz-binsz,'width',ww/2,'colors',mycc.green_light);
setBoxStyle(h,linew)
h = boxplot(cos_sim_avg.svd(:,2),'positions',2*stepsz+binsz,'width',ww/2,'colors',mycc.green);
setBoxStyle(h,linew)
h = boxplot(cos_sim_avg.louvain(:,1),'positions',3*stepsz-binsz,'width',ww/2,'colors',mycc.purple_light);
setBoxStyle(h,linew)
h = boxplot(cos_sim_avg.louvain(:,2),'positions',3*stepsz+binsz,'width',ww/2,'colors',mycc.purple);
setBoxStyle(h,linew)
h = boxplot(cos_sim_avg.osi(:,1),'positions',4*stepsz-binsz,'width',ww/2,'colors',mycc.gray_light);
setBoxStyle(h,linew)
h = boxplot(cos_sim_avg.osi(:,2),'positions',4*stepsz+binsz,'width',ww/2,'colors',mycc.gray);
setBoxStyle(h,linew)
xlim([0 5*stepsz]); ylim([0 0.5])
set(gca,'xcolor','w')
ylabel('similarity')
...
setBoxStyle(h,linew)
h = boxplot(auc.louvain,'positions',3*stepsz,'width',ww,'colors',mycc.purple);
setBoxStyle(h,linew)
h = boxplot(auc.osi,'positions',4*stepsz,'width',ww,'colors',mycc.gray);
setBoxStyle(h,linew)
xlim([0 5*stepsz]); ylim([0 1])
ylabel('AUC')
set(gca,'xcolor','w')
box off
pval = zeros(1,2);
pval(1) = ranksum(auc.crf,auc.svd);
pval(2) = ranksum(auc.crf,auc.louvain);
pval(3) = ranksum(auc.crf,auc.osi);
title([num2str(pval(1)) ', ' num2str(pval(2)) ', ' num2str(pval(3))])
...
setBoxStyle(h,linew)
h = boxplot(pred_stats.louvain(:,1),'positions',3*stepsz,'width',ww,'colors',mycc.purple);
setBoxStyle(h,linew)
h = boxplot(pred_stats.osi(:,1),'positions',4*stepsz,'width',ww,'colors',mycc.gray);
setBoxStyle(h,linew)
xlim([0 5*stepsz]); ylim([0 1])
ylabel('accuracy')
set(gca,'xcolor','w')
...
pval(2) = ranksum(pred_stats.crf(:,1),pred_stats.louvain(:,1));
pval(3) = ranksum(pred_stats.crf(:,1),pred_stats.osi(:,1));
title([num2str(pval(1)) ', ' num2str(pval(2)) ', ' num2str(pval(3))])
...
setBoxStyle(h,linew)
h = boxplot(pred_stats.louvain(:,2),'positions',3*stepsz,'width',ww,'colors',mycc.purple);
setBoxStyle(h,linew)
h = boxplot(pred_stats.osi(:,2),'positions',4*stepsz,'width',ww,'colors',mycc.gray);
setBoxStyle(h,linew)
xlim([0 5*stepsz]); ylim([0 1])
ylabel('precision')
set(gca,'xcolor','w')
...
pval(2) = ranksum(pred_stats.crf(:,2),pred_stats.louvain(:,2));
pval(3) = ranksum(pred_stats.crf(:,2),pred_stats.osi(:,2));
title([num2str(pval(1)) ', ' num2str(pval(2)) ', ' num2str(pval(3))])
...
setBoxStyle(h,linew)
h = boxplot(pred_stats.louvain(:,3),'positions',3*stepsz,'width',ww,'colors',mycc.purple);
setBoxStyle(h,linew)
h = boxplot(pred_stats.osi(:,3),'positions',4*stepsz,'width',ww,'colors',mycc.gray);
setBoxStyle(h,linew)
xlim([0 5*stepsz]); ylim([0 1])
ylabel('recall')
set(gca,'xcolor','w')
...
pval(2) = ranksum(pred_stats.crf(:,3),pred_stats.louvain(:,3));
pval(3) = ranksum(pred_stats.crf(:,3),pred_stats.osi(:,3));
title([num2str(pval(1)) ', ' num2str(pval(2)) ', ' num2str(pval(3))])

saveas(gcf,[fig_path expt_ee '_svd_louvain_osi_core_pred_' ge_type '_stats.pdf'])
