function model = treegmFitBinary(X,  values, obs, obsType, weights)
...
if nargin < 3, obs = []; end
if nargin < 4, obsType = 'none'; end
if nargin < 5, weights = ones(1,Ncases); end
...
[mi, nmi, pij, pi] = mutualInfoAllPairsDiscreteBinary(X, values, weights); %#ok
%[mi] = mutualInfoAllPairsDiscrete(X, domain, weights);
[adjmat, ~] = minSpanTreePrim(-mi); % find max weight spanning tree
