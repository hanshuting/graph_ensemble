...
//       tol = config.GetDouble("L2N2_DaiFletcherPGM.tolerance");
   ...
   if(verbosity > 0)
      std::cout << "L2N2_DaiFletcherPGM instantiated!" << std::endl;
}
                                    ...
                                    double *u, double *x, double &lam_ext)
{
   ...
   //if (fabs(r) < DaiFletcherPGM::tol_r) return 0;
   
   // chteo: inequality constraint, i.e., with \xi >= 0 constraint
   if (r < DaiFletcherPGM::tol_r) return 0;
   ...
   lam_ext = lambda;
   if(innerIter >= maxProjIter && verbosity >=2)
