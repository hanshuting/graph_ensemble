...
// Added by Kui Tang, 21 Aug 2014
#ifdef PRINT_DESCRIBE_SELF
puts("csa.hh compilation description:");
for (int j = 0; desc[j] != NULL; j++) {
  puts(desc[j]);
}
#endif
...
struct ParseException : std::exception {
  int err_index_;
  ParseException(int err_index) : err_index_(err_index) { }
//  const char *what() const _NOEXCEPT { return err_messages[(err_index_ % ERRBASE) - 1]; }
  char *what() { return err_messages[(err_index_ % ERRBASE) - 1]; }
};

void parse_error(int err_index)

{
  throw ParseException(err_index);
}
...
for (unsigned i = 0; i < m; i++) {
	...
	tail = temp;
      ...
      // Added by Kui Tang 28 Aug 2014
      /*
      fprintf(stderr, "i = %d, graph[i] = [%d %d %d], tail = %d, head = %d, id_offset = %d, cost = %d, m = %d, n = %d, lhs_n = %d\n",
          i,
          graph[3*i+0], graph[3*i+1], graph[3*i+2],
          tail, head, id_offset, cost, m, n ,lhs_n);
      fprintf(stderr, "(tail < 1)=%d, (tail > lhs_n)=%d, (head < 1)=%d, (head > n - lhs_n)=%d\n",
          (tail < 1), (tail > lhs_n), (head < 1), (head > n - lhs_n));
          */
