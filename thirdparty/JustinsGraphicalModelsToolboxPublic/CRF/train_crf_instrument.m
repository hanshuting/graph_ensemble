function [p, histMinFunc, sumTs] = train_crf_instrument(feats,efeats,labels,models,loss_spec,crf_type,options)

% *KT* ported instrumentation code from train_labelers_instrument_reg.m
% *KT* added trace of sum(T)
...
function outputFcn(x,optimValues,state,varargin)
    ...
    if strcmp(state, 'iter')
        ...
        if mod(optimValues.iteration, checkpoint_interval) == 0 && ~isempty(checkpoint_file)
