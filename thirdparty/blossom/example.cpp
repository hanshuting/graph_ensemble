...
#ifndef PERFECT_MATCHING_DOUBLE
...
int main(int argc, char* argv[])
{
	...
	if (!geom_filename)
	{
      ...
      //printf("%s:%d -- Added (%d, %d, %d)\n", __FILE__, __LINE__, edges[2*e], edges[2*e+1], weights[e]);
    }

    //printf("%s:%d -- options\n", __FILE__, __LINE__);
    //puts(print_options(options).c_str());

    //printf("%s:%d -- interrogation\n", __FILE__, __LINE__);
    //printf(interrogate(*pm).c_str());
