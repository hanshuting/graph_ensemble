function [bnryStates,nodeNames] = getDFBnryStates(rawTSMat,tikName)

%Quantization based on first order differences.
...
coreDFTS = diff(coreTS);%.^(1./(diff(coreTimeStps)/dT)) - 1;
coreTimeStps = coreTimeStps(2:end); %abandon the first time stamp to accomodate for percentage change



%%%%%%%%Default quantization is based on deciles.%%%%%%
coreEFIStates = discEFI(coreDFTS, {[0.1:0.1:1]});
