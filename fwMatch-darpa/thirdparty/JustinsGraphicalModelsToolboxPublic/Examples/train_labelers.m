function [params times fvals] = train_labelers(loss_spec,params0,testmode,...
    imsdir,labelsdir,nvals,feat_params,edge_params,maxims,maxtime)
...
function [L grad E] = eval(params,N,nograd)
    if toc(time0)>maxtime
        warning('KT: eval exceeded maxtime %g\n', maxtime);
        L = 0;
...
%         norm(efeats-efeats2)
    ...
    fprintf('KT: Inside eval; reg = %g\ngrad \t= [%s]\nparams \t= [%s]\nscaling \t= [%s]\n', ...
        reg, num2str(grad'), num2str(params'), num2str(scaling'));
...
if testmode
reg = 0;
[L grad E] = eval(params,1:length(Aims),1); % 1 turns off gradient computation
fprintf('loss_spec: %s  L: %f  E: %f\n', loss_spec, L, E); %E/length(Aims)
