...
void TheMatrix::copy_thematrix(const TheMatrix& tm, const int& _dense_or_sparse)
{
        ...
        switch(tm.type) 
        {
                case SML::DENSE_VECTOR: 
                        switch(type) 
                        {
                                case SML::DENSE_VECTOR: dv = new ublas::vector<Scalar>(*(tm.dv)); break;
                                case SML::SPARSE_VECTOR: sv = new ublas::compressed_vector<Scalar>(*(tm.dv)); break;
                        }
                        break;
                ...
                case SML::SPARSE_VECTOR:
                        switch(type) 
                        {
                                case SML::DENSE_VECTOR: dv = new ublas::vector<Scalar>(*(tm.sv)); break;
                                case SML::SPARSE_VECTOR: sv = new ublas::compressed_vector<Scalar>(*(tm.sv)); break;
                        }
                        break;
                ...
                case SML::DENSE_MATRIX: 
                        switch(type) 
                        {
                                case SML::DENSE_MATRIX: dm = new ublas::matrix<Scalar>(*(tm.dm)); break;
                                case SML::SPARSE_MATRIX: sm = new ublas::compressed_matrix<Scalar>(*(tm.dm)); break;
                        }
                        break;
                ...
                case SML::SPARSE_MATRIX: 
                        switch(type) 
                        {
                                case SML::DENSE_MATRIX: dm = new ublas::matrix<Scalar>(*(tm.sm)); break;
                                case SML::SPARSE_MATRIX: sm = new ublas::compressed_matrix<Scalar>(*(tm.sm)); break;
                        }
                        break;
        }
     ...
     smr(0)
{
        ...
        switch(vector_or_matrix)
        {
                case SML::VECTOR: 
                        ...
                        {
                                case SML::DENSE:
                                        dv = new ublas::vector<Scalar>(length);
                                        dv->clear();
                                        type = SML::DENSE_VECTOR;  
                                        break;
               
                                case SML::SPARSE:
                                        sv = new ublas::compressed_vector<Scalar>(length,non_zeros);
                                        sv->clear();
                ...
                case SML::MATRIX:
                        ...
                        {
                                case SML::DENSE:
                                        dm = new ublas::matrix<Scalar>(row,column);
                                        dm->clear();
                                        type = SML::DENSE_MATRIX;
                                        break;
               
                                case SML::SPARSE:
                                        sm = new ublas::compressed_matrix<Scalar>(row,column,non_zeros);
                                        sm->clear();
 ...
 */
void TheMatrix::Dot(const TheMatrix& other, Scalar& output) const
{
 ...
 *    \param output - [write] The result of the dot product
 */
void TheMatrix::TransposeDot(const TheMatrix& other, Scalar& output) const
{
        // pointer to member function
...
void TheMatrix::SumRows(TheMatrix& output)
{
        ...
        //chteo: [301207:2247] use identity_vector<Scalar> one instead of TheMatrix
}
 ...
 *    \param output - [write] The result of the dot product
 */
void TheMatrix::dv_dot_v(const TheMatrix& other, Scalar& output) const
{
 ...
 *    \param output - [write] The result of the dot product
 */
void TheMatrix::sv_dot_v(const TheMatrix& other, Scalar& output) const
{
   int otherType = other.Type();
 ...
 *    \param output - [write] The result of the dot product
 */
void TheMatrix::smr_dot_v(const TheMatrix& other, Scalar& output) const
{
        int otherType = other.Type();
 ...
 *    \param output - [write] The result of the dot product
 */
void TheMatrix::dmr_dot_v(const TheMatrix& other, Scalar& output) const
{
        int otherType = other.Type();
                ...
                case SML::SPARSE_MATRIX_ROW:
                        ...
                        switch(outputType) 
                        {
                                case SML::DENSE_VECTOR:  //axpy_prod(*dm, *(other.smr), *(output.dv)); break;
                                                         noalias(*(output.dv)) = prod(*dm, *(other.smr)); break; // this is much faster!
 ...
 *    This function is here for consistency purpose.
 *
 ...
 */
void TheMatrix::v_addrow_dense(const unsigned int& rowidx, const unsigned int& length, const Scalar* values)
{
 ...
 */
void TheMatrix::v_addrow_sparse(const unsigned int& rowidx, const unsigned int& length, const Scalar* values, const unsigned int* indices)
{
        assert(indices);
        v_addrow_dense(rowidx, length, values);
}
 ...
 */
void TheMatrix::v_getrow_dense(const unsigned int& rowidx, unsigned int& length, Scalar* values) const
{
   assert(type == SML::DENSE_VECTOR || type == SML::SPARSE_VECTOR);
   assert(length == 1);
   assert(values);   
 ...
 */
void TheMatrix::v_getrow_sparse(const unsigned int& rowidx, unsigned int& length, Scalar* values, unsigned int* indices) const
{
        assert(indices);
        v_getrow_dense(rowidx, length, values);
        indices[0] = 0;
}
 ...
 *    \param values  - [read] The row elements
 */
void TheMatrix::dm_addrow_dense(const unsigned int& rowidx, const unsigned int& length, const Scalar* values)
{
 ...
 *    \param indices - [read] The column indices corresponding to the values 
 */
void TheMatrix::dm_addrow_sparse(const unsigned int& rowidx, const unsigned int& length, const Scalar* values, const unsigned int* indices)
{
        assert(type == SML::DENSE_MATRIX);
        assert(length <= this->column);
        assert(values);
 ...
 *    \param values  - [read] The row elements
 */
void TheMatrix::sm_addrow_dense(const unsigned int& rowidx, const unsigned int& length, const Scalar* values)
{
 ...
 *    \param indices - [read] The column indices corresponding to the values 
 */
void TheMatrix::sm_addrow_sparse(const unsigned int& rowidx, const unsigned int& length, const Scalar* values, const unsigned int* indices)
{
        assert(type == SML::SPARSE_MATRIX);
        assert(length <= this->column);
        assert(values);
 ...
 *    \param values  - [read] The row elements
 */
void TheMatrix::sm_setrow_dense(const unsigned int& rowidx, const unsigned int& length, const Scalar* values)
{
        assert(type == SML::SPARSE_MATRIX);
        assert(length == this->column);
        assert(values);
 ...
 *    \param indices - [read] The column indices corresponding to the values 
 */
void TheMatrix::sm_setrow_sparse(const unsigned int& rowidx, const unsigned int& length, const Scalar* values, const unsigned int* indices)
{
        assert(type == SML::SPARSE_MATRIX);
        assert(length <= this->column);
        assert(values);
 ...
 *    \param values  - [write] The values in the row (memory preallocated)
 */
void TheMatrix::dm_getrow_dense(const unsigned int& rowidx, unsigned int& length, Scalar* values) const
{
        assert(type == SML::DENSE_MATRIX);
        assert(values);
        assert(rowidx < row);
        ...
        length = this->column;        
        memcpy(values, &((*dm).data()[start_idx]), sizeof(Scalar)*length);        
}
 ...
 *    \param indices - [write] The column indices of the values in the row (memory preallocated)
 */
void TheMatrix::dm_getrow_sparse(const unsigned int& rowidx, unsigned int& length, Scalar* values, unsigned int* indices) const
{
        assert(type == SML::DENSE_MATRIX);
        assert(values);
        assert(indices);
 ...
 *    \param values  - [write] The values in the row (memory preallocated)
 */
void TheMatrix::sm_getrow_dense(const unsigned int& rowidx, unsigned int& length, Scalar* values) const
{
        assert(type == SML::SPARSE_MATRIX);
        assert(values);
        assert(rowidx < row);
        ...
        length = this->column;
        memset(values,0,sizeof(Scalar)*length);
 ...
 *    \param indices - [write] The column indices of the values in the row (memory preallocated)
 */
void TheMatrix::sm_getrow_sparse(const unsigned int& rowidx, unsigned int& length, Scalar* values, unsigned int* indices) const
{
        assert(type == SML::SPARSE_MATRIX);
        assert(values);
        assert(indices);
...
TheMatrix* TheMatrix::CreateMatrixRowView(const unsigned int& index)
{
        ...
        {
                case SML::SPARSE_MATRIX:
                        mat_row->smr = new ublas::matrix_row<ublas::compressed_matrix<Scalar> >(*sm,index);
                ...
                case SML::DENSE_MATRIX:
                        mat_row->dmr = new ublas::matrix_row<ublas::matrix<Scalar> >(*dm,index);
