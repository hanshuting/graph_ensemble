...
void lap(int size, cost **C, int *col, int *row)
{
  int i,j,k,l;
  int m = size, n = size;
  cost s;
  ...
  cost *row_dec = (cost*)malloc(sizeof(cost)*m); 
  for (i=0;i<m;++i) 
    row_dec[i] = 0;
  cost *col_inc = (cost*)malloc(sizeof(cost)*n); 
  for (i=0;i<n;++i) 
    col_inc[i] = 0;
  cost *slack = (cost*)malloc(sizeof(cost)*n);  
  for (i=0;i<n;++i) 
    slack[i] = 0;
