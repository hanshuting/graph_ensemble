...
for I = 0:(SprinklerNetwork.nrFactors()-1)
    PJ = PJ * SprinklerNetwork.factor(I);
end
PJ.normalize(); % Not necessary: a Bayesian network is already normalized by definition

% Calculate some probabilities
denom = PJ.marginal(dai.VarSet(W));
denom = denom(1);
var1 = PJ.marginal(dai.VarSet(S,W));
var2 = PJ.marginal(dai.VarSet(R,W));
fprintf('P(W=1) = %f\n', denom);
fprintf('P(S=1 | W=1) = %f\n', var1(3) / denom);
fprintf('P(R=1 | W=1) = %f\n', var2(3) / denom);
